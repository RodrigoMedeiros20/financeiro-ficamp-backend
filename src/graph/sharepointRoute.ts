...
sharepointRouter.get("/contas", exigeLogin, async (req, res) => {
  try {
    ...
    const r = fresh ? await obterContasFresco(token) : await obterContas(token);
    res.json({ contas: r.contas, doCache: r.doCache, idadeMs: r.idadeMs });
  } catch (e: any) {
